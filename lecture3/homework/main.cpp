...
// 绘制可视化识别结果
void draw_armor(cv::Mat& img, auto_aim::Armor& armor)
{
    cv::Scalar color;
    switch (armor.color)
    {
    ...
    const cv::Point2f text_point = armor.points[0] + cv::Point2f(0, -12);
    tools::draw_text(img, armor_text, text_point, color, 1.0, 2);
}
...
int main()
{
    ...
    io::myCamera camera(10000, 20, 20);
